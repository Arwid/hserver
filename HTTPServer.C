...
bool HTTPServer::doTerminate = false;

void HTTPServer::HUP_received(int val)
{
	doTerminate = true;
	cout << "HUP_received";
}
...
HTTPServer::HTTPServer(Thread *thread, ssbuf * ssbuffer_, Socket* socket): Runnable(thread) { //Socket* socket):Runnable(thread){
	index1 = 0;
	con = Config::Instance();

	Port = atoi(con->lookupParam("Port").c_str());
	ROOT_DIR = con->lookupParam("DocRoot");
	masterSocket = socket;
	tempnum = con->saveParam();
	ssbuffer = ssbuffer_;
...
string HTTPServer:: lookupUserParam(string name) {
	for(int i = 0; i<index1; i++)
		if(name == usernames[i]) return uservalues[i];
	return "";
}

string HTTPServer:: strconvert (int i) { 
...
int HTTPServer:: FileSize (const char * filename) { 
...
void HTTPServer:: httperror (iostream &sstream, int status, string s_str, string relURL) { 
...
void HTTPServer:: makeDirectory (iostream &sstream, string relURL) {
...
int HTTPServer::Run() {
	...
	//Mutex * mutex = MutexInstance::Instance();
	cout << "Running a new" << endl;
	iostream sstream;
